...
test("timeout rejects the pending expect and forgets it", async () => {
	...
	await assert.rejects(
		pending.result,
		/no ControlResult for cmd=2 within 1234ms/,
	);
...
test("cancel frees the slot so a retry, not the dead attempt, gets the result", async () => {
	...
	assert.equal(
		c.deliver({ v: 1, ok: true, cmd: COMMAND.LINK, message: "linked" }),
		true,
	);
...
test("confirmsLink accepts only ok results that reflect the requested link", () => {
	...
	assert.equal(
		confirmsLink({ ok: true, cmd: 1, link_group_id: 777, link_slot: 1 }, want),
		true,
	);
	// A stale ok result from an earlier attempt describes a different slot/group.
	assert.equal(
		confirmsLink({ ok: true, cmd: 1, link_group_id: 777, link_slot: 2 }, want),
		false,
	);
	assert.equal(
		confirmsLink({ ok: true, cmd: 1, link_group_id: 999, link_slot: 1 }, want),
		false,
	);
	assert.equal(
		confirmsLink({ ok: false, cmd: 1, error: "link_conflict" }, want),
		false,
	);
