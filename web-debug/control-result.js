	...
	return {
		...
		expect(cmd) {
			...
			entry.timer = setTimeoutFn(() => {
				if (remove(entry)) {
					entry.reject(
						new Error(`no ControlResult for cmd=${cmd} within ${timeoutMs}ms`),
					);
				}
			}, timeoutMs);
			pending.push(entry);
			return {
				...
				cancel() {
					...
					entry.reject(
						new Error(`ControlResult wait for cmd=${cmd} cancelled`),
					);
				},
			};
		},
...
export function confirmsLink(result, { groupId, slot }) {
	return (
		result?.ok === true &&
		result.link_group_id === groupId &&
		result.link_slot === slot
	);
}
