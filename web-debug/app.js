...
async function connectBluetoothDevice(bluetoothDevice) {
	...
	resultChar.addEventListener("characteristicvaluechanged", (event) => {
		...
		refreshInfo(entry).catch((err) =>
			log(`refresh info failed: ${err.message}`),
		);
		render();
	});
...
async function reconnectGrantedDevices() {
	...
	for (const bluetoothDevice of granted) {
		try {
			...
			log(
				`reconnect failed: ${bluetoothDevice.name || bluetoothDevice.id}: ${err.message}`,
			);
		}
	}
}
...
async function linkAsSlot(entry, slot) {
	const result = await writeConfirmed(
		entry,
		COMMAND.LINK,
		slot,
		getForceFlag(),
	...
	if (!result.ok) {
		log(
			`LINK slot ${slot} rejected by ${entry.info.device_id}: ${describeControlError(result)}`,
		);
		render();
		return;
	}
...
async function unlinkDevice(entry) {
	...
	if (!result.ok) {
		log(
			`UNLINK rejected by ${entry.info.device_id}: ${describeControlError(result)}`,
		);
		render();
		return;
	}
...
function slotSummary(slot) {
	...
	const okLink =
		state && state.linkGroupId === pair.groupId && state.linkSlot === slot;
...
function renderDeviceCard(entry) {
	...
	const title =
		entry.info?.name || entry.bluetoothDevice.name || entry.bluetoothDevice.id;
	...
	node.querySelector(".deviceHash").textContent =
		entry.info?.device_hash ?? "-";
	...
	node.querySelector(".pressed").textContent = entry.state?.pressed
		? "YES"
		: "no";
...
function init() {
	...
	$("#reconnectGrantedBtn").disabled =
		!hasWebBluetooth || !navigator.bluetooth.getDevices;

	$("#addDeviceBtn").addEventListener("click", () =>
		addDevice().catch(showError),
	);
