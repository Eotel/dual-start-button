...
struct FakeEffects final : ControlEffects {
  ...
  void applyLinkState(const LinkState& state) override {
    char buf[64];
    snprintf(buf, sizeof(buf), "apply g=%lu s=%u gen=%lu armed=%d", static_cast<unsigned long>(state.group_id),
             state.slot, static_cast<unsigned long>(state.generation), state.armed ? 1 : 0);
  ...
  void emitControlResult(bool ok, uint8_t cmd, const char* error_code, const char* message) override {
    (void)message;
    char buf[64];
    snprintf(buf, sizeof(buf), "result ok=%d cmd=%u err=%s", ok ? 1 : 0, cmd, error_code ? error_code : "-");
    events.push_back(buf);
  }
  ...
  void blinkStatus(uint8_t r, uint8_t g, uint8_t b, uint32_t duration_ms) override {
    char buf[48];
    snprintf(buf, sizeof(buf), "blink %u,%u,%u,%lu", r, g, b, static_cast<unsigned long>(duration_ms));
    events.push_back(buf);
  }
};

static FakeEffects replay(uint8_t command, uint8_t slot, uint8_t flags, uint32_t group_id, uint32_t value,
                          const LinkState& current) {
...
static void assert_events(const std::vector<std::string>& expected, const FakeEffects& fx) {
  TEST_ASSERT_EQUAL_UINT32_MESSAGE(expected.size(), fx.events.size(), "unexpected side-effect count");
...
void test_link_accept_persists_then_result_blink_publish() {
  const FakeEffects fx = replay((uint8_t)ControlCommand::Link, 1, 0, 999, 0, LinkState{0, 0, 0, true});
  assert_events(
      {
...
void test_unlink_clear_persists_then_blink_publish_result() {
  const FakeEffects fx = replay((uint8_t)ControlCommand::Unlink, 0, 0, 777, 0, LinkState{777, 1, 4, true});
  assert_events(
      {
...
void test_unlink_noop_skips_persist_and_blink() {
  const FakeEffects fx = replay((uint8_t)ControlCommand::Unlink, 0, 0, 777, 0, LinkState{0, 0, 0, true});
  assert_events(
      {
...
void test_set_armed_false_publishes_state_without_persist() {
  const FakeEffects fx = replay((uint8_t)ControlCommand::SetArmed, 0, 0, 0, 0, LinkState{777, 1, 4, true});
  assert_events(
      {
...
void test_link_conflict_reject_keeps_state_and_publishes_error() {
  const FakeEffects fx = replay((uint8_t)ControlCommand::Link, 1, 0, 999, 0, LinkState{777, 1, 4, true});
  assert_events(
      {
